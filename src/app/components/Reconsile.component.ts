...
export class ReconcileComponent{
  ...
  hasReconActivityData:boolean = false;
  ...
  buildReconActivityTableData(){
    let base = this;
    this.hasReconActivityData = false;
    ...
    setTimeout(function(){
      base.hasReconActivityData = true;
    });
  };
  ...
  filterByPeriod(event) {
    ...
    if(event.target.value) {
      let date = this.dateFormater.formatDate(event.target.value, this.dateFormat, this.serviceDateformat);
      this.reconcileService.filterReconActivity(date)
        .subscribe(reconActivityData => {
          this.reconActivity = reconActivityData;
          this.buildReconActivityTableData();
          this.loadingService.triggerLoadingEvent(false);
