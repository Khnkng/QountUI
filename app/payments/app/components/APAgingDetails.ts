...
export class APAgingDetails  {
  ...
  constructor(private _router: Router,private excelService:ExcelService, private reportService: ReportService,private emailService:EmailService,private _toastService: ToastService, private switchBoard: SwitchBoard,private emailBuilder: FormBuilder) {
  ...
  convertTabletoJSON(headerSelector:string, selector:string){
    ...
    if($header.length!=0){
      ...
      $header.each(function( thr ) {
        ...
        headerArr.push($cell.text());
        // combinedHeaderText += $cell.text() +"\n";
        headerStyle = styleObj;

      });
      combinedHeaderText = headerArr.join("\n");
      let rowObj = {}, tdObj = {};
      rowObj['row0'] = {};
  ...
  doEmail(event){
    ...
    var reportElem = jQuery("#displayReport");
    reportElem.wrapInner("<center></center>");
    if(jQuery("#displayReport table.report-table th:first img").length==0){
      var imgStr = "<img src='"+this.apAgingReportLogoUrl+"' width='20' />";
      jQuery("#displayReport table.report-table th:first").append(imgStr);
    }
    var pdfHtmlStr = "<html><head><style>.columns,table{min-width:initial}@page{size:A4 landscape}thead{background:#f8f8f8 }td,th{line-height:30px;text-align:left}.report-heading{text-align:center;font-size:20px;color:#282895 ;font-weight:700}.display-report{border:1px solid #e3e5e5 ;text-align:center;padding-top:10px}.report-table tr td{border-top:1px solid #0a4b3e ;border-bottom:1px solid #0a4b3e }table{width:100%;margin-bottom:1rem;border-radius:3px;border-collapse:collapse;border-spacing:0;display:table;border-color:grey}tr.last td{border-top:1px solid #0a4b3e ;border-bottom:1px solid #0a4b3e }</style></head><body>";
  ...
  onChange(deviceValue) {
    ...
    if(deviceValue=='Potrait') {
      ...
      document.getElementById("sucess").style.height = "600px"
      document.getElementById("sucess").style.width = "750px";
    }
    else{
