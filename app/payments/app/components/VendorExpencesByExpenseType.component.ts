...
export class VendorExpencesByExpenseType {
  companies:any=[];
  report:any={};
  ...
  constructor(private _router: Router,private emailService:EmailService,private excelService:ExcelService,private _toastService: ToastService,private reportService: ReportService, private switchBoard: SwitchBoard,private emailBuilder: FormBuilder) {
  ...
  generateReport(data){
    ...
    if(report.daysPerAgingPeriod&&report.numberOfPeriods&&report.asOfDate)
      this.reportService.generateReport(report).subscribe(report  => {
        ...
        this.resetData();
        this.expenseDetails=report.data;
        this.columns=report.columns;
        this.groupBy=report.expenses;
        this.isDisplay=true;
